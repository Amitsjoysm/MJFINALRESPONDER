...
class DateParserService:
    ...
    def parse_time_references(self, text: str) -> List[Tuple[str, datetime, str]]:
        ...
        week_range_matches = re.finditer(r'(?:in|after)\s+(\d+)\s*[-â€“]\s*(\d+)\s+weeks?', text, re.IGNORECASE)
        ...
        day_range_matches = re.finditer(r'(?:after|in)\s+(\d+)\s*[-â€“]\s*(\d+)\s+days?', text, re.IGNORECASE)
    ...
    def _parse_flexible_date(self, date_text: str, current_date: datetime) -> Optional[datetime]:
        ...
        day_range_match = re.search(r'(\d+)\s*[-â€“]\s*(\d+)\s+days?', date_text_lower)
        if day_range_match:
        ...
        week_range_match = re.search(r'(\d+)\s*[-â€“]\s*(\d+)\s+weeks?', date_text_lower)
        if week_range_match:
